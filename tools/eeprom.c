...
void eeprom_init()
{
    ...
    while(!eeprom_valid) {
        eeprom_enable = i2c_data_read(eeprom_address, 0, &eeprom_data, 1);
//    
...
{
    return i2c_data_read(eeprom_address, dat_addr, data, len);
...
bool eeprom_data_write(uint8_t dat_addr, uint8_t* data, uint8_t len)
{
    ...
    return i2c_block_write(eeprom_address, eeprom_data, len + 1, true);
}

bool eeprom_byte_write(uint8_t byte_addr, uint8_t byte)
{
    uint8_t sb = byte;
    return eeprom_data_write(byte_addr, &sb, 1);
}