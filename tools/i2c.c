...
static volatile bit sda;

void i2c_init()
{
    TRISB7 = 0; //set RB7 as output
    TRISB6 = 0; //set RB6 (SDA) as output
    OPTION_REGbits.nRBPU = 1; //PORTB pull up(1) / no(0)
   /* 
    char data[2];
...
#define i2c_stop()      TRISB6 = 0; SDA_SET(0); TRISB7 = 1; I2C_DELAY(2);\
                        TRISB6 = 1; I2C_DELAY(2)//; GIE = 1

#define i2c_start()     i2c_stop();  TRISB7 = 1; TRISB6 = 1; I2C_DELAY(1);\
                        TRISB6 = 0; SDA_SET(0); I2C_DELAY(1); SCL_DOWN();\
                        TRISB7 = 0

#define i2c_clock()     I2C_DELAY(1); TRISB7 = 1; I2C_DELAY(2); \
                        TRISB7 = 0; SCL_DOWN();I2C_DELAY(1)   //i2cdelay(I2CCLOCKLOW);         //-- Minimum Clock Low Time
...
bool i2c_getack(void)
{
    ...
    SCL_DOWN();//SCL=I2CLOW;
//TRISB6 = 0;
//I2C_DELAY(8);//i2cdelay(I2CHALFCLOCK);          //-- wait for 1/2 the clock pulse
//di();
...
char i2c_getbyte(void)
{
    ...
    for(count = 8;count > 0; count--) {   //-- Read 8 bits of data
        ...
        TRISB7 = 1;//SCL_TRIS=I2CHIGH;              //-- Release clock
        I2C_DELAY(1); //i2cdelay(I2CHALFCLOCK);        //-- 1/2 Minimum Clock High Time
        if(SDA_GET != 0) Byte += 1;//if(SDA !=0 ) Data=1;           //-- READ in the data bit
        I2C_DELAY(1); //i2cdelay(I2CHALFCLOCK);        //-- 1/2 Minimum Clock High Time
        TRISB7 = 0;//SCL_TRIS=I2CLOW;               //-- Lower the clock
        I2C_DELAY(2); //i2cdelay(I2CCLOCKLOW);         //-- Minimum Clock Low Time
        /*
 ...
 */   
    ...
    if(!i2c_getack()) result = true;// false;
    i2c_stop();
